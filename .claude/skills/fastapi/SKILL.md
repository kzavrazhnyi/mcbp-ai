...
офіційний FastAPI скіл, FastAPI CLI команди, fastapi dev, fastapi run, Annotated залежності FastAPI, Pydantic моделі запитів, SSE стрімінг ендпоінт, StreamingResponse FastAPI, async ендпоінти FastAPI, маршрутизатори prefix tags, lifespan FastAPI, response_model фільтрація
